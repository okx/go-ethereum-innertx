...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	...
	for i, tx := range block.Transactions() {
		...
		receipt, err, innerTxs, erc20s := applyTransaction(msg, p.config, nil, gp, statedb, blockNumber, blockHash, tx, usedGas, vmenv)
		//add InnerTx
		if innerTxs != nil {
...
func applyTransaction(msg types.Message, config *params.ChainConfig, author *common.Address, gp *GasPool, statedb *state.StateDB, blockNumber *big.Int, blockHash common.Hash, tx *types.Transaction, usedGas *uint64, evm *vm.EVM) (*types.Receipt, error, []*vm.InnerTx, []*vm.ERC20Contract) {
	...
	*usedGas += result.UsedGas
	...
	innerTxs, erc20s := afterApplyTransaction(evm, result.Failed())
	return receipt, err, innerTxs, erc20s
}
...
func checkTransaction(vmenv *vm.EVM, failed bool) []*vm.InnerTx {
	if failed == true {
		...
		return vmenv.InnerTxies
	} else if len(vmenv.InnerTxies) > 1 {
		return vmenv.InnerTxies
	} else {
		return nil
	}
